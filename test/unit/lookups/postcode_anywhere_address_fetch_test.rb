...
class PostcodeAnywhereAddressFetchTest < GeocoderTestCase
  ...
  def test_result_components_with_fetch
    results = Geocoder.search('address here', fetch_id: '50782641.00')
  ...
  def test_no_results
    assert_equal [], Geocoder.search('no results', fetch_id: 1)
  end
  ...
  def test_key_limit_exceeded_error
    ...
    assert_raises Geocoder::OverQueryLimitError do
      Geocoder.search('key limit exceeded', fetch_id: 1)
    end
  end
  ...
  def test_unknown_key_error
    ...
    assert_raises Geocoder::InvalidApiKey do
      Geocoder.search('unknown key', fetch_id: 1)
    end
  end
  ...
  def test_generic_error
    ...
    exception = assert_raises(Geocoder::Error) do
      Geocoder.search('generic error', fetch_id: 1)
    end
